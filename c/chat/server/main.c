...
int main(int argc, char *argv[])
{
	...
	if(db == NULL)
	{
		// Print an error message and quit if databse fails to open
		fprintf(stderr, "%s: %s sqlite: could not open database %s\n", SERVER_NAME, ERROR_MSG, DB_FILE);
		exit(-1);
	}

	// Create a query to truncate the users table in the databsae on open
	...
	{
		// On query failure, print a console error and exit
		fprintf(stderr, "%s: %s sqlite: could not truncate users table\n", SERVER_NAME, ERROR_MSG);
		exit(-1);
	}
...
void *tcp_listen()
{
   	...
   	while(1)
	{
		...
		else
		{
			...
			thpool_add_work(threadpool, &chat, (void*)&inc_fd);
		}
	}
}
